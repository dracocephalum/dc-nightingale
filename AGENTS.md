...
following the client pattern of the reference event store.

Primary framework: .NET. Namespace prefix: `Dracocephalum`.
...
| Projects, assemblies, namespaces | PascalCase, prefixed | `Dracocephalum.ProxyGateway` |
| Test projects | `test/<Project>.Tests/` mirrors `src/<Project>/` | `Dracocephalum.ProxyGateway.Tests` |

A trailing `.Core` names the assembly, not the namespace — `Dracocephalum.Core` has
namespace `Dracocephalum` — applied by `Directory.Build.props`; the full rule and
