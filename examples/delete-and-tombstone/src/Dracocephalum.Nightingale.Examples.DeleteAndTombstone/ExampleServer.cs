...
public sealed class ExampleServer : IAsyncDisposable
{
    ...
    /// <param name="allowDeletion">Whether the server allows delete and tombstone; the default host allows neither.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The running server; dispose it to stop it.</returns>
    public static async Task<ExampleServer> StartAsync(string connectionString, bool allowDeletion, CancellationToken cancellationToken)
    {
        ...
        {
            options.Deletion.AllowDelete = allowDeletion;
            options.Deletion.AllowTombstone = allowDeletion;
        });
